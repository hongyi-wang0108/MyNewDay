package com.example.mynewday.datepicker;
...
public class CustomDatePicker implements View.OnClickListener, PickerView.OnSelectListener {
     ...
     * 时间单位：时、分
     */
    private static final int SCROLL_UNIT_HOUR = 0b1;//1
    private static final int SCROLL_UNIT_MINUTE = 0b10;//2
    ...
    private void initView() {
        ...
        if (window != null) {//设置pickerdialog的窗口
    ...
    public void setCancelable(boolean cancelable) {
        if (!canShow())//先判断一下 可以show吗，就是自身加载好了吗，
                       // 有错误吗那些，自身没问题了，才能讨论用户让不让show
            return;
    ...
    private void initScrollUnit(Integer... units) {//没看懂
        if (units == null || units.length == 0) {//不显示时分
            mScrollUnits = SCROLL_UNIT_HOUR + SCROLL_UNIT_MINUTE;
        } else {
            for (int unit : units) {//1 2
                mScrollUnits ^= unit;//^异或
              //  mScrollUnits = unit^mScrollUnits
            }
        }
    }
